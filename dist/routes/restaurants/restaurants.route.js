...
const restaurants_mod_1 = require("../../restaurants/restaurants.mod");
const restify_errors_1 = require("restify-errors");
class RestaurantRouter extends model_router_1.ModelRouter {
    constructor() {
        super(restaurants_mod_1.Restaurant);
        this.findMenu = (req, resp, next) => {
            restaurants_mod_1.Restaurant.findById(req.params.id, "+menu")
                .then(rest => {
                if (!rest) {
        ...
        this.replaceMenu = (req, resp, next) => {
            restaurants_mod_1.Restaurant.findById(req.params.id)
                .then(rest => {
                if (!rest) {
                    throw new restify_errors_1.NotFoundError('Restaurant not found');
                }
